...
namespace System.Drawing.Html
{
    ...
    public class CssBlock
    {
        ...
        public CssBlock(string blockSource,int a)
        : this()
        {
            _block = blockSource;
            ...
            foreach (Match match in matches)
            {
                ...
                //Register only if property checks with reflection
                if (CssBoxControl._properties.ContainsKey(propName))
                    PropertyValues.Add(CssBoxControl._properties[propName], propValue);
            }
        }
        #endregion
        ...
        internal void UpdatePropertyValuesControl()
        {
            PropertyValues.Clear();

            foreach (string prop in Properties.Keys)
            {
                if (CssBoxControl._properties.ContainsKey(prop))
                    PropertyValues.Add(CssBoxControl._properties[prop], Properties[prop]);
            }
        }
        /// <summary>
        ...
        /// </summary>
        /// <param name="b"></param>
        public void AssignTo(CssBoxControl b)
        {
            foreach (PropertyInfo prop in PropertyValues.Keys)
            {
                string value = PropertyValues[prop];
