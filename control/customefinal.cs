...
{
    public partial class customefinal : UserControl
    {
       public inith inithh;
        ...
        public customefinal(string getsett, RecevDataFromWeb ee)
        {
            InitializeComponent();
            evn = ee;
            inithh = new inith(continers, continers.ThemeName, getsett,evn);
            
          // inslisprovid();
        }
        private void continers_Click(object sender, EventArgs e)
        {
        ...
        string them;
        public customefinal(string allcon, string them)
        {
            InitializeComponent();
            this.allcon = allcon;
            this.them = them;
                ...
                if (response.StatusCode == System.Net.HttpStatusCode.Accepted|| response.StatusCode == System.Net.HttpStatusCode.OK|| response.StatusCode == System.Net.HttpStatusCode.Created)
                {
                    ...
                    try
                    {
                        ...
                        if (getresponse.Count > 0)
                        {
                            ...
                            {
                                dd.messge_aler(continers.ThemeName, 2, 0, JsonConvert.SerializeObject(r), "message");

                            }
                            else
                            {
                                dd.messge_aler(continers.ThemeName, 2, 0, response.Content, "message");

                            }
                        }
                    }
                    catch (Exception ex) {
                        dd.messge_aler(continers.ThemeName, 2, 0,ex.Message, "message");
                ...
                if (ii != null)
                {
                    ...
                    if (getresponse.Count > 0)
                    {
                        ...
                        if (selcta.ToString().ToLower().StartsWith("t"))
                        {
                            ...
                            if (IsValidJson(data))
                            {
                                dd.messge_aler(continers.ThemeName, 1, 0, JsonConvert.SerializeObject(r), "message");
                               
                            }
                            else {
                                dd.messge_aler(continers.ThemeName, 1, 0, data, "message");
       ...
       private List<hedaervalue> anotherdataget(string daiop)
        ...
        //            Radconverttojson f = new Radconverttojson(ghh, radValidationRule1, imagesvg,fromhtmlh,evn,this);
